...
const ptComponents = {
  ...
  block: {
    h1: ({ children }: any) => (
      <h1 className="text-6xl mt-5 font-light">{children}</h1>
    ),
    h2: ({ children }: any) => (
      <h2 className="text-5xl mt-5 font-light">{children}</h2>
    ),
    h3: ({ children }: any) => (
      <h3 className="text-4xl mt-5 text-gray-700 font-light">{children}</h3>
    ),
  },
  types: {
    image: ({ value }: any) => {
      ...
      <article className="flex flex-col items-center gap-5 z-10 relative">
        ...
        <div className="max-w-4xl w-[90vw] md:w-[70vw] lg:w-[50vw] flex flex-col gap-2">
          ...
          <motion.h3
            layout="position"
            layoutId={slug}
            className="font-light text-3xl md:text-4xl xl:text-5xl"
          >
            {title}
          </motion.h3>
        ...
        <div className="max-w-6xl w-[90vw] md:w-[80vw] lg:w-[60vw] flex flex-col md:flex-row md:flex-[1_0] gap-5">
          ...
          {/* Description */}
          <div className="font-light text-gray-600 basis-[40%] text-lg">
            <p>{description}</p>
          </div>
        </div>
        ...
        {/* Bottom half */}
        {/* Content */}
        <div className="max-w-5xl w-[90vw] md:w-[70vw] lg:w-[50vw] flex flex-col gap-2 text-lg leading-9 font-light">
